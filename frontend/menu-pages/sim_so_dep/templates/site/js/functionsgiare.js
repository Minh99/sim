...
function addCustomPriceToList(label, value) {
    // $(".read-more").data('page', 1);
    var html = `<div class="wraper-item-select"><div class="item-checked customPrice" id="${value}" type="3">${label}<i class="fa-solid fa-xmark remove-item-filter"></i></div></div>`;
    if ($('.list_filter_item').find('.customPrice').length > 0) {
...
function post_filter(array) {
    ...
    array.push([6, $('.sort-price').data('action')])
        ...
        success: function(result) {
            ...
            if (result.data.length > 0) {
                result.data.map((elm, idx) => {
                    var nhamang = elm.mang;
                    var url = window.location.origin + `/dat-sim-phong-thuy-${elm.sim}.html`;
                    html_list += `
...
$('.list_filter_item').on('click', '.remove-item-filter', function() {
    ...
    $('.checkedfilterall').find('#' + $(this).parent().attr('id')).parent().remove()
...
$('.reset').click(function() {
    ...
    if (parent == 1) {
        $('.so-tranh').find('.active').removeClass('active')
    }
    if (parent == 2) {
        $('.networkAll').find('.active').removeClass('active')
        $('.select-item-network').find('.active').removeClass('active')
    }
    if (parent == 3) {
        $('.priceAll').find('.active').removeClass('active')
    }
    if (parent == 4) {
        $('.numberPrefixAll').find('.active').removeClass('active')
    }
    if (parent == 5) {
        $('.sim-types').find('.active').removeClass('active')
        $('.types-select').find('.active').removeClass('active')
    }
...
$('#checkGiua').click(function() {
    $("#checkDuoi").prop("checked", false);
    let params = get_arr_filter();
    doCall(params);
});
$('#checkDuoi').click(function() {
    $("#checkGiua").prop("checked", false);
    let params = get_arr_filter();
    doCall(params);
});
...
$('.show-filter-total').click(function() {
    if (!$(this).hasClass('checked')) {
        $('.show-total').addClass('show')
        $('body').css('overflow', 'hidden')
        $('body').addClass('bg-black')
        $(this).addClass('checked')
    } else {
        $('.show-total').removeClass('show')
...
$('.close-btn').click(function() {
    ...
    $('body').css('overflow', 'auto')
    $('body').removeClass('bg-black')
    $('.show-filter-total').removeClass('checked')
})
$('.button-total-fix').on('click', '.result', function() {
    $('.show-total').removeClass('show')
    $('body').css('overflow', 'auto')
    $('body').removeClass('bg-black')
    $('.show-filter-total').removeClass('checked')
})
$('.show-total-main').scroll(function() {
    if ($(this).scrollTop() > 40) {
        $('.bgr-while-fix').css('display', 'none')
    } else {
        $('.bgr-while-fix').css('display', 'block')
    }
})
$('.reset-all').click(function() {
    $('.checkedfilterall').children().not($('.remove-all')).remove();
    $('.selected-item-filter').children().children().not($('.remove-all')).remove();

    $('.show-total-main').find('.active').removeClass('active')
    $('.wapper_scroll').find('.active').removeClass('active')
